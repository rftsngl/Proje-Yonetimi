          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-[32px] bg-white shadow-2xl shadow-indigo-900/10">
            ...
            <div className="flex shrink-0 items-center gap-2 border-b border-slate-100 bg-slate-50/50 px-8 py-3 overflow-x-auto no-scrollbar">
              {TABS.map((tab) => {
                ...
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`group relative flex items-center gap-2 whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold transition-all ${isActive ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-slate-100' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'}`}>
                    <Icon className={`h-4 w-4 ${isActive ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'}`} />
                    {tab.label}
                    {tab.count !== undefined && tab.count > 0 && (
                      <span className={`ml-1.5 rounded-full px-2 py-0.5 text-[10px] ${tab.countColor || 'bg-slate-100 text-slate-600 group-hover:bg-slate-200'}`}>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            {teamMembers.map((member) => (
                              <div key={member.id} className="flex items-center gap-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
      ...
      {stakeholders.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {stakeholders.map((s: any) => editingId === s.id ? (
            ...
            <div key={s.id} className="group relative rounded-2xl border border-slate-100 bg-white p-6 shadow-sm hover:border-indigo-100 transition-colors">
              {onUpdateProject && (
      ...
      {costItems.length > 0 ? (
        <div className="space-y-3">
          {costItems.map((c: any) => editingId === c.id ? (
            ...
            <div key={c.id} className="group flex items-center justify-between rounded-xl border border-slate-100 bg-white p-4 shadow-sm transition-all hover:border-indigo-100">
              <div>
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 space-y-6">
      ...
      {plans.length > 0 ? (
        <div className="space-y-3">
          {plans.map((plan: any) => (
            <div key={plan.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              {editingId === plan.id ? (
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
